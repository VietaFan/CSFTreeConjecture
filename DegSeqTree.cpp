...
/*
int countTrees(int n, int verbose) {
	vector<int*> trees;
	vector<int> degSeq;
	if (verbose)
		cout << "Counting trees on " << n << " vertices...\n";
	clock_t t = clock();
	vector<vector<int> > vec = getPartitions(n-2,n-2);
	if (verbose >= 2)
		cout << vec << endl;
	...
	for (auto degSeq: vec) {
		trees.clear();
		generateTrees(degSeq, trees);
		for (int *p: trees) {
			delete[] p;
		}
		if (verbose >= 3)
			cout << "There are " << trees.size() << " trees with degree sequence " << degSeq << ".\n";
		if (verbose >= 4) {
	...
	if (verbose) {
		cout << "Time: " << t << " ms.\n";
		cout << "Total tree count: " << total << endl;
	}
	return total;
}
